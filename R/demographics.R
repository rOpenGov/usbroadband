demographics <- function(geographyType=NULL, geographyId=NULL, geographyName=NULL, lat=NULL, long=NULL, version=NULL, all=FALSE, ...){
    ...
    if(!is.null(lat) & !is.null(lat))
        ...
        if(!is.null(geographyId))
            out <- bb(api=paste('demographic', version, geographyType, 'ids',
                                paste(head(geographyId,10),collapse=','), sep='/'),
                      args=c(format='json'), ...)
        if(!is.null(geographyName))
            out <- bb(api=paste('demographic', version, geographyType, 'names',
                                paste(head(geographyName,10),collapse=','), sep='/'),
                      args=c(format='json'), ...)
        else
